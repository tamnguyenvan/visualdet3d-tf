...
class Anchors(layers.Layer):
    ...
    def anchors_to_indexes(self, anchors: np.ndarray) ->Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def call(self, image: tf.Tensor, calibs: List[np.ndarray]=[], is_filtering=False):
        ...
        if self.shape is None or not any(shape == self.shape):
            ...
            if isinstance(image, tf.Tensor):
                ...
                self.anchors = tf.constant(all_anchors.astype(np.float32))
            elif isinstance(image, np.ndarray):
                self.anchors = tf.convert_to_tensor(all_anchors.astype(np.float32))
            self.anchors_image_x_center = tf.reduce_mean(self.anchors[0,:,0:4:2], axis=1) #[N]

...
from visualdet3d.utils import theta_to_alpha_3d
...
class ConvertToFloat(object):
    ...
    def __call__(self,
                 ...
                 image_gt=None,
                 lidar=None):
        return (left_image.astype(np.float32),
                right_image if right_image is None else right_image.astype(np.float32),
                p2,
                p3,
                labels,
                image_gt,
...
class Normalize(object):
    ...
    def __call__(self, left_image, right_image=None, p2=None, p3=None, labels=None, image_gt=None, lidar=None):
        ...
        left_image /= np.tile(self.std, int(left_image.shape[2]/self.std.shape[0]))
        ...
        if right_image is not None:
            ...
            right_image /= np.tile(self.std, int(right_image.shape[2]/self.std.shape[0]))
            right_image = right_image.astype(np.float32)
...
class RandomSaturation(object):
    ...
    def __call__(self, left_image, right_image=None, p2=None, p3=None, labels=None, image_gt=None, lidar=None):
        if np.random.rand() <= self.distort_prob:
            ratio = np.random.uniform(self.lower, self.upper)
            left_image[:, :, 1] *= ratio
            if right_image is not None:
                right_image[:, :, 1] *= ratio
            ...
            if labels:
                if isinstance(labels, list):
                    ...
                    for obj in labels:
                        ...
                        # alpha 
                        obj.alpha = theta_to_alpha_3d(ry, obj.x, z, p2)
...
class RandomHue(object):
    ...
    def __call__(self, left_image, right_image=None, p2=None, p3=None, labels=None, image_gt=None, lidar=None):
        if np.random.rand() <= self.distort_prob:
            shift = np.random.uniform(-self.delta, self.delta)
            left_image[:, :, 0] += shift
...
class Shuffle(object):
    ...
    """
    def __init__(self, aug_list:List[edict]):
        self.transforms = [
            build_single_transform(aug_cfg) for aug_cfg in aug_list
        ]
...
class Compose(object):
    ...
    def __init__(self, aug_list: List[edict], is_return_all=True):
...
def build_single_transform(cfg: edict):
    """
    """
    name = list(cfg.keys())[0]
    keywords = cfg[name]

    transform = globals().get(name, None)
